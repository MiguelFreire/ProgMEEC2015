...
void RenderTable(int , int [], TTF_Font *, SDL_Surface **, SDL_Renderer * );
...
void dealCards(int player_cards[], int * player_hand ,int deck[], int cards);
void resetGame(int player_cards[][MAX_CARD_HAND], int player_hand[]);
...
int main( int argc, char* args[] )
{
    ...
    int numDecks=0 , SeedMoney=0, BetValue=0;
    ...
    shuffleDeck(deck, numDecks); //Create house deck
   ...
   for(int i = 0; i <= 1; i++) {
     ...
     for(int p = 0; p <= 3; p++) {
       if(!i) money[p] = SeedMoney;
       dealCards(player_cards[p], &pos_player_hand[p], deck, 1);
     }
     dealCards(house_cards, &pos_house_hand, deck, 1);
    }
    pos_house_hand = 1; 
 	while( quit == 0 )
    {
      ...
      while( SDL_PollEvent( &event ) )
        {
            ...
            break;
					case SDLK_h:
                        ...
                        // todo
          case SDLK_n:
          resetGame(player_cards, pos_player_hand);
          shuffleDeck(deck, numDecks);
          playersTurn=0; //Create house deck
          for(int p = 0; p <= 3; p++) {
            dealCards(player_cards[p], &pos_player_hand[p], deck, 1);
          }
          printf("Player1 carta 0: %d\n", player_cards[0][0]);
          printf("Player1 carta 1: %d\n", player_cards[0][1]);
          printf("Player2 carta 0: %d\n", player_cards[1][0]);
          printf("Player2 carta 1: %d\n", player_cards[1][1]);
          printf("Player3 carta 0: %d\n", player_cards[2][0]);
          printf("Player3 carta 1: %d\n", player_cards[2][1]);
          printf("Player4 carta 0: %d\n", player_cards[3][0]);
          printf("Player4 carta 1: %d\n", player_cards[3][1]);
          break;
					default:
						break;
				}
        ...
        // render game table
        RenderTable(playersTurn, money, serif, imgs, renderer);
...
void setConfig(int *numDecks, int *SeedMoney, int *BetValue) {
    ...
    scanf("%d", BetValue);
    while(*BetValue < 1 || *BetValue > (0.20 * *SeedMoney)) {
      printf("Valor de cada aposta não pode ser inferior a 1 nem a 20%% do dinheiro inicial (%.2f)!\n", 0.20**SeedMoney);
      printf("Valor de aposta?\n");
      scanf("%d", BetValue);
    }
...
void createDeck(int deck[], int numDecks) {
  int k = 0;
  for(int m = 0; m <= numDecks-1 ; m++) {
    for(int n = 0; n <= MAX_DECK_SIZE-1; n++,k++) {
      deck[k] = n;
    }
  }
}
...
void dealCards(int cards[], int * hand ,int deck[], int numCards) {
 ...
 */
void RenderTable(int playersTurn, int _money[], TTF_Font *_font, SDL_Surface *_img[], SDL_Renderer* _renderer)
{
    ...
    {
      if(playersTurn == i) SDL_SetRenderDrawColor(_renderer, 0, 255, 0, 255 );
